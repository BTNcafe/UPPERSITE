 ...
 * UPPERSITE Packer.
 */
global.PACKER_UPPERSITE = function() {'use strict';
	...
	// log.
	log = function(msg) {
		console.log('PACKER FOR UPPERSITE: ' + msg);
	},
	...
	scanBoxFolder = function(func) {
		...
		fs.readdirSync(boxName).forEach(function(name) {
			if (name[0] !== '.' && name !== 'BROWSER' && name !== 'COMMON' && name !== 'SERVER' && name !== 'OCTOPUS' && name !== 'WEB') {
				func(boxName + '/' + name);
			}
		});
	},
	...
	scanResourceFolder = function(func) {
		...
		fs.readdirSync(boxName + '/WEB').forEach(function(name) {
			if (name[0] !== '.' && name !== 'BROWSER_FIX') {
				func(boxName + '/WEB/' + name);
			}
		});
	},
	...
	scanFolderForBrowserFix = function(path, func) {
		...
		if (fs.existsSync(path) === true) {
			...
			fs.readdirSync(path).forEach(function(name) {
				...
				if (name[0] !== '.' && fs.statSync(rootPath + '/' + fullPath).isDirectory() === true && fullPath !== boxName + '/WEB/BROWSER_FIX/IE/LIB') {
					folderPaths.push(fullPath);
				} else if (fullPath === boxName + '/WEB/BROWSER_FIX/IE/LIB' || fs.statSync(rootPath + '/' + fullPath).isDirectory() !== true) {
					func(fullPath);
				}
			});
	...
	fs.mkdirSync('__PACK/' + boxName + '/WEB');
	copyFolderWithMinify(boxName + '/WEB', '__PACK/' + boxName + '/WEB');
